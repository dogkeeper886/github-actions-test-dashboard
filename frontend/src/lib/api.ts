...
export const workflowsApi = {
  ...
  getWorkflowRuns: (
  ...
  ): Promise<{ runs: WorkflowRun[]; pagination: unknown }> => {
    ...
    return apiClient.get(
      `/workflows/${workflowId}/runs${query ? `?${query}` : ""}`,
    );
  },
  ...
  getJobLogs: async (runId: string, jobId: number): Promise<string> => {
    const response = await fetch(
      `${API_BASE_URL}/runs/${runId}/jobs/${jobId}/logs`,
    );
    if (!response.ok) throw new Error(`API Error: ${response.status}`);
    return response.text();
  },

  refreshData: (): Promise<unknown> => apiClient.post("/refresh/collect"),
