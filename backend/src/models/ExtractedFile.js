...
class ExtractedFile {
  ...
  static async findByRunIdGrouped(runId) {
    ...
    files.forEach(file => {
      const category = file.file_type === 'image' ? 'images' :
                     file.file_type === 'json' ? 'json' :
                     file.file_type === 'text' ? 'text' : 'binary'
  ...
  static async findById(id) {
    const db = await getDatabase()
    const result = await db.query('SELECT * FROM extracted_files WHERE id = $1', [id])
    return result.rows[0]
  }
